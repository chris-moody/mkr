describe("rgplygn basics v"+mkr.constructs.rgplygn.VERSION, function() {
  var m, a;

  beforeAll(function() {
    m = new mkr({css:{width:300, height:300, x:300, position:'relative'}});
    a = m.construct('rgplygn', {
      radius:50, sides:5
    });
  });
  ...
  describe("Instantiation", function() {
    ...
    it("create a new rgplygn", function() {
      expect(a).toBeDefined();
    });
    it("every new rgplygn has a polygon element", function() {
      expect(a.el.constructor.name).toEqual('SVGPolygonElement');
    });

    it("a read-only id and coords array", function() {
      expect(a.id).toEqual('rgplygn-0');
      a.id = 'someNewId';
      expect(a.id).toEqual('rgplygn-0');

      expect(a.coords.length).toEqual(5);
      a.coords = [];
      expect(a.coords.length).toEqual(5);
    });
    ...
    it("read/write x, y, sides and r", function() {
      ...
      expect(a.sides).toBeDefined();
      a.sides = 3;
      expect(a.sides).toEqual(3);

      expect(a.r).toBeDefined();
      a.r = 50;
      expect(a.r).toEqual(50);
    });
  ...
  describe("Instance methods...", function() {
    it("rgplygn can get a point", function() {
