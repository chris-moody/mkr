describe("plyln basics v"+mkr.constructs.plyln.VERSION, function() {
  var m, a;
  ...
  describe("Instantiation", function() {
    ...
    it("create a new plyln", function() {
      expect(a).toBeDefined();
    });
    it("every new plyln has a polygon element", function() {
      expect(a.el.constructor.name).toEqual('SVGPolylineElement');
    });

    it("a read-only id", function() {
      expect(a.id).toEqual('plyln-0');
      a.id = 'someNewId';
      expect(a.id).toEqual('plyln-0');
    });
  ...
  describe("Instance methods...", function() {
    it("plyln can add a point", function() {
      a.addPoint([0, 5])
